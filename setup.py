...
trift_sources = all_sources + []

all_lib_dirs = []
all_include = [numpy_include, "./include", "./delaunator-cpp/include"]
all_libs = []
...
if run_cuda_install:
    ...
    gpu_extension_dict = dict(
        sources=trift_sources + gpu_sources,
...
shutil.copy("trift/gputrift.pyx", "trift/trift.pyx")
shutil.copy("trift/gputrift.pxd", "trift/trift.pxd")
...
cpu_extra_compile_args = {"gcc": ["-std=c++11", '-Ofast', '-march=native', "-fPIC", '-fopenmp']}

cpu_extra_link_args = ["-std=c++11",'-Ofast','-march=native', '-fopenmp']

cpu_extension_dict = dict(
    sources=trift_sources,
    ...
    include_dirs=all_include,
)

cpu_extensions = []

temp_dict = copy.deepcopy(cpu_extension_dict)
extension_name = "trift"
folder = "trift/"
temp_dict["sources"] += [folder + extension_name + ".pyx"]

cpu_extensions.append(Extension("trift.trift", **temp_dict))
